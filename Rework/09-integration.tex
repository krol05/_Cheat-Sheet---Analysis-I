...
$\displaystyle\int f'e^f\dx=e^f+C$.\\
...
\item Apply, repeat if the new integral is still a product ($x^2e^x$: twice).
...
\item Multiply out and compare coefficients, or plug in the roots (\hd{cover-up} is fastest for simple linear factors).
...
\item $\left|\int_a^bf\right|\le(b-a)\max|f|$ (the standard estimate).
\item Period $T$: $\int_a^{a+T}f$ does not depend on $a$.
...
\item General interval: $\frac{b-a}n\sum f\!\left(a+k\frac{b-a}n\right)\to\int_a^bf$.
\end{rcp}
...
\begin{rcp}
\item Find every point where the sign or the active branch changes (zeros of the inside).
\item Split there, drop the $|\cdot|$ with the correct sign on each piece.
...
$f$ (bounded) is integrable $\iff\sup_PL=\inf_PU$ $\iff\forall\eps>0\ \exists P:U(f,P)-L(f,P)<\eps$.\\
...
\begin{bul}
\item \yes\ continuous on $[a,b]$ \ra\ integrable. \ Monotone on $[a,b]$ \ra\ integrable. \ Differentiable \ra\ continuous \ra\ integrable.
\item \yes\ bounded with \emph{finitely} (even countably) many discontinuities \ra\ integrable. \ Lebesgue criterion: bounded and the discontinuity set is a null set $\iff$ integrable.
\item \no\ ``must be continuous everywhere'' --- jumps are fine.
\item \no\ ``integrable \ra\ differentiable'' --- a step function is integrable and not even continuous.
\item \no\ ``finitely many function \emph{values} \ra\ integrable'' --- Dirichlet $1_\Q$ takes only $0,1$ and is not integrable.
...
\item Write each as a limit: $\int_a^\infty f=\lim_{R\to\infty}\int_a^Rf$, \ $\int_a^bf=\lim_{\eps\to0^+}\int_a^{b-\eps}f$.
\item Compare with the benchmark below at that specific point.
