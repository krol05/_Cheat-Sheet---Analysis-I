...
\begin{rcp}
\item Get the pointwise limit $f$ first --- uniform convergence can only be towards it.
...
\item The max being attained at a \emph{different} point $x_n$ for each $n$ is irrelevant. Only the \emph{value} $M_n$ counts.
\item $f_n\to f$ pointwise with all $f_n$ and $f$ continuous still need not be uniform ($nxe^{-nx^2}$ on $[0,1]$).
\end{bul}
}
...
\item On $[a,b]$: $f$ is Riemann integrable and $\int_a^bf_n\to\int_a^bf$.
\item $f_n$ bounded uniformly \ra\ $f$ bounded.
