...
\end{rcp}
\hd{Same thing as ``variation of constants'':} $y_h=Ce^{-\int p}$; let $C=C(x)$, insert, get $C'=qe^{\int p}$, integrate.
}
\E{$y'-\frac2xy=x^3$}{
...
\item Find all roots (guess an integer root, then polynomial division).
...
\F{Wronskian: is my fundamental system valid?}{
$W(x)=\det\begin{pmatrix}y_1&y_2\\y_1'&y_2'\end{pmatrix}=y_1y_2'-y_1'y_2$.\\
$W(x_0)\ne0$ for one $x_0$ \ra\ $y_1,y_2$ are linearly independent \ra\ they span all solutions.
For $y''+by'+cy=0$: $W(x)=W(0)e^{-bx}$, so $W$ is either never zero or identically zero (Abel).
}
\F{reduction of order (one solution known)}{
Given one solution $y_1$ of $y''+p y'+q y=0$, set $y_2=v(x)y_1$. Inserting kills the $v$ term and leaves a first-order ODE for $v'$:
$v'=\frac1{y_1^2}e^{-\int p}$, so $y_2=y_1\int\frac{e^{-\int p}}{y_1^2}\dx$.
}
...
\begin{rcp}
\item Pick the shape from the table above.
...
\F{variation of constants (when no ansatz fits, e.g.\ $g=\frac1x$, $\tan x$)}{
2nd order with $y_h=C_1y_1+C_2y_2$: set $y_p=c_1(x)y_1+c_2(x)y_2$ with
\[c_1'=-\frac{y_2g}W,\qquad c_2'=\frac{y_1g}W,\qquad W=y_1y_2'-y_1'y_2,\]
then integrate. Always works; just slower than the ansatz.
}
...
\item Solutions can never cross an equilibrium (uniqueness), so each one is trapped between two of them and converges monotonically.
\end{rcp}
}
