...
\item \hd{Telescoping?} $a_n=b_n-b_{n+1}$ \ra\ $s_N=b_1-b_{N+1}$.
\end{rcp}
}
...
\X{test traps}{
\begin{bul}
\item Leibniz needs \emph{monotone} $b_n$: $b_n=\frac{2+(-1)^n}n\to0$ but is not monotone.
\item Comparison needs $a_n\ge0$; with signs, apply it to $|a_n|$ (that proves \emph{absolute} convergence only).
\item $\sum a_n$ conv.\ and $\sum b_n$ conv.\ \ra\ $\sum(a_n+b_n)$ conv.\ \yes, \ but $\sum a_nb_n$ conv.\ \no\ ($a_n=b_n=\frac{(-1)^n}{\sqrt n}$).
\item ``$s_N$ bounded \ra\ converges'' \no\ unless $a_n\ge0$ (then $s_N$ is monotone).
\end{bul}
}
...
\hd{Recipe:} partial-fraction it, write out the first three and last two terms, see what survives.
}
