...
\item \hd{Radius} (Cauchy--Hadamard): $\frac1R=\lim\left|\frac{a_{n+1}}{a_n}\right|$ for factorials, or $\frac1R=\limsup\sqrt[n]{|a_n|}$ for powers. Limit $0$ \ra\ $R=\infty$; limit $\infty$ \ra\ $R=0$.
...
\end{rcp}
\hd{Order bookkeeping:} $\sin x=x+O(x^3)$, so $\frac{\sin x}x=1+O(x^2)$. Track $O(x^k)$ explicitly and you cannot lose a term.
}
...
\begin{bul}
\item $\frac{1-\cos x}{x^2}=\frac{x^2/2+O(x^4)}{x^2}\to\frac12$.
...
\item $\frac{\ln(1+x)-x}{x^2}\to-\frac12$.
\end{bul}
}
...
\item \hd{Divide:} long division of the series, or multiply by the geometric expansion of the denominator.
\item \hd{Shift centre:} to expand at $x_0\ne0$, substitute $t=x-x_0$ and expand in $t$.
\end{rcp}
}
\F{ready-made compositions}{
$\ln(1+x^2)=x^2-\frac{x^4}2+\frac{x^6}3-\dots$;\quad
$\sin(x^2)=x^2-\frac{x^6}6+\dots$;\quad
$e^{-x^2}=1-x^2+\frac{x^4}2-\dots$;\\
$\frac1{1+x^2}=1-x^2+x^4-\dots$;\quad
$\sqrt{1+x}=1+\frac x2-\frac{x^2}8+\frac{x^3}{16}-\dots$;\quad
$\frac1{\sqrt{1-x}}=1+\frac x2+\frac{3x^2}8+\dots$
}
