...
\begin{rcp}
\item \hd{Inner function linear, $ax+b$:} $\displaystyle\int f(ax+b)\dx=\frac1aF(ax+b)+C$ --- divide by the inner derivative $a$.
\item \hd{Inner function anything else:} you may \textbf{not} patch with a factor. Substitute $u=g(x)$, $\dx=\frac{\du}{g'(x)}$, and continue only if the leftover $g'(x)$ cancels.
...
\item \hd{Differentiate your answer.} The chain-rule factor must reappear.
\end{rcp}
...
\end{itemize}}
\hd{Forgot one?} Derive it from $e^{ix}$ (\S\ref{sub:cxforms}) in ten seconds.
}
