...
$f:[a,b]\to\Rl$ \textbf{continuous}, $[a,b]$ \textbf{compact} \de{closed and bounded, \S\ref{sub:quant}}:
\begin{bul}
...
\begin{bul}
\item \claimF{``IVT always gives a solution''} --- $p\equiv-1$ has none, since $e^x>0$.
\item \claimT{``$p$ of \emph{odd} degree \ra\ always at least one solution''} --- $e^x-p(x)$ changes sign at $\pm\infty$.
\item \claimT{``at most finitely many solutions for every $p$''} --- $e^x-p(x)$ is analytic and not identically $0$, so its zeros are isolated and finite in number on $\Rl$ here.
\item \claimT{``for every $N$ there is a $p_N$ with $\ge N$ solutions''} --- take $p_N$ close to $e^x$ on a long interval and let it wobble across.
\end{bul}
}
