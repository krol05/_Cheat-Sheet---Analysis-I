...
\tsThe{(Divergence Test / Necessary Condition for Convergence)}{
    If the series $\sum_{n=0}^{\infty} a_n$ converges, then necessarily
    \(
        \lim_{n\to\infty} a_n = 0.
    \)
    \tsPoint The converse is false: the harmonic series $\sum_{n=1}^{\infty}\frac1n$ has $\frac1n\to0$ but diverges ($\sum_{n=1}^\infty\frac1n=\infty$).
}
...
\tsThe{(Monotone Convergence for Series / Non-Negative Terms)}{
...
\tsThe{(Absolute Convergence $\Rightarrow$ Convergence)}{
...
\tsThe{(Comparison Test / Majorant--Minorant Criterion)}{
...
\tsThe{(p-Series and Geometric Series --- Comparison Benchmarks)}{
    \(
        \sum_{n=1}^{\infty} \frac{1}{n^p}
    \)
    converges iff $p>1$ (p-series).
    \tsPoint
    \(
        \sum_{n=0}^{\infty} r^n
    \)
    converges iff $|r|<1$, with sum $\frac{1}{1-r}$; more generally $\sum_{n=0}^\infty ar^n=\frac{a}{1-r}$.
    \tsPoint Finite geometric sum: $\sum_{k=0}^{n} r^k = \frac{1-r^{n+1}}{1-r}$ for $r\neq1$.
}
\tsThe{(Telescoping Sums)}{
    \(
        \sum_{k=0}^{n} (b_k - b_{k+1}) = b_0 - b_{n+1}.
    \)
    If $b_n \to L$, then $\sum_{k=0}^{\infty} (b_k-b_{k+1}) = b_0 - L$.
}
\tsThe{(Cauchy Root Test)}{
...
\tsThe{(Leibniz Alternating Series Test)}{
...
\tsThe{(Dirichlet's Rearrangement Theorem)}{
...
\tsThe{(Cauchy Product / Mertens' Theorem)}{
...
\tsThe{(Cauchy--Hadamard Theorem / Radius of Convergence)}{
