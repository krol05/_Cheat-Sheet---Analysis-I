...
\tsDef{Classification of Discontinuities}{
    At a point $x_0\notin$ continuity, $f$ has:
    \begin{itemize}[noitemsep]
        \item a \emph{removable} discontinuity if $\lim_{x\to x_0^-}f(x)=\lim_{x\to x_0^+}f(x)$ exists but $\neq f(x_0)$ (or $f(x_0)$ undefined);
        \item a \emph{jump} discontinuity if both one-sided limits exist but differ;
        \item an \emph{essential/infinite} discontinuity if some one-sided limit fails to exist or is $\pm\infty$.
    \end{itemize}
}
