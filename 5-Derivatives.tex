...
\tsThe{Basic Derivatives}{
    Basic Derivatives --- full table on p.\pageref{tab:derivatives}
    \begin{itemize}[noitemsep]
        \item Power:
              \(
                  (ax^p)' = a p x^{p-1}
...
\tsThe{Fermat's Theorem (Necessary Condition for Extremum)}{
    ...
    \(
        f'(x_0) = 0.
    \)
    \tsPoint The converse is false: $f'(x_0)=0$ does not imply an extremum, e.g.\ $f(x)=x^3$ at $x_0=0$.
}
...
\tsThe{l'Hospital's Rule --- Case $0/0$}{
    If
    \(
...
\tsThe{l'Hospital's Rule --- Case $\infty/\infty$}{
    If
    \(
...
\tsThe{l'Hospital's Rule --- At Infinity}{
    If
    \(
...
\tsIdea{Reducing Other Indeterminate Forms to $0/0$ or $\infty/\infty$}{
    \begin{itemize}[noitemsep]
        \item $0\cdot\infty$: write $fg=\frac{f}{1/g}$ or $\frac{g}{1/f}$.
        \item $\infty-\infty$: combine over a common denominator, e.g.\ $f-g=\frac{1/g-1/f}{1/(fg)}$.
        \item $0^0,\ 1^\infty,\ \infty^0$: write $f^g=e^{g\ln f}$, then find $\lim g\ln f$ (a $0\cdot\infty$ form).
    \end{itemize}
    Once reduced to $0/0$ or $\infty/\infty$, apply l'Hospital's Rule.
}
...
\)
\tsPoint If $f''(x_0)=0$, the test is inconclusive (e.g.\ $f(x)=x^4$ at $0$ is a minimum, $f(x)=x^3$ at $0$ is neither).
}
\tsDef{Inflection Point}{
$x_0$ is an \emph{inflection point} of $f$ if $f''$ changes sign at $x_0$ (convexity flips to concavity or vice versa). $f''(x_0)=0$ alone is not sufficient, e.g.\ $f(x)=x^4$ has $f''(0)=0$ but no inflection at $0$.
}
