...
\end{rcp}
\hd{``Variation of constants'' \de{Variation der Konstanten} is the same method, spelled out:} solve the homogeneous problem first, $y_h=Ce^{-\int p}$; then \textbf{promote the constant to a function} $C=C(x)$ and insert $y=C(x)e^{-\int p}$ into the ODE. Everything cancels except $C'=q\,e^{\int p}$, which you integrate. Use the name if the exam asks for it; the integrating factor gets you there faster.
}
\E{$y'-\frac2xy=x^3$}{
...
$W(x_0)\ne0$ for one $x_0$ \ra\ $y_1,y_2$ are \textbf{linearly independent} \ra\ they span all solutions.
For $y''+by'+cy=0$: $W(x)=W(0)e^{-bx}$, so $W$ is either never zero or identically zero (Abel).
}
\F{reduction of order (one solution known)}{
\textbf{Given one solution $y_1$} of $y''+p y'+q y=0$, set $y_2=v(x)y_1$. Inserting kills the $v$ term and leaves a \textbf{first-order ODE for $v'$}:
...
then integrate. \textbf{Always works}; just slower than the ansatz.
}
